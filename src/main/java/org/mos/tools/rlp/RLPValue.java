...
public class RLPValue implements ActorService {
    ...
    public static RLPValue fromRlpEncoded(byte[] data) {

        if (data != null && data.length != 0) {
            RLPValue v = new RLPValue();
            v.init(data);
            return v;
        }
        return null;
    ...
    public RLPValue(Object obj) {

        this.decoded = true;
        if (obj == null) return;

        if (obj instanceof RLPValue) {
            this.value = ((RLPValue) obj).asObj();
        } else {
            this.value = obj;
        }
    }

    public RLPValue withHash(byte[] hash) {
        sha3 = hash;
        return this;
    }
    ...
    public int countBranchNodes() {
        ...
        if (this.isList()) {
            ...
            for (Object obj : objList) {
                i += (new RLPValue(obj)).countBranchNodes();
            }
            return i;
